...
describe('DashboardMascotas AdminShell migration', () => {
  it('uses the shared shell while preserving pet ownership and history', () => {
    ...
    expect(mascotas).toContain('Cargando mascotas...</p>')
    expect(mascotas).toContain('Cargando historial...</p>')
    expect(mascotas).not.toContain('loadError instanceof Error ? loadError.message')
    expect(mascotas).not.toContain('historyError instanceof Error ? historyError.message')
    expect(mascotas).not.toContain('deleteError instanceof Error ? deleteError.message')
    expect(mascotas).toContain('saveError instanceof MascotaError ? saveError.message :')
