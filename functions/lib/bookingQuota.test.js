...
describe('consumeBookingAttempt', () => {
    ...
    it('increments attempts while the window is active', () => {
        expect(consumeBookingAttempt({ windowStartedAt: now, attempts: 2 }, new Date(now.getTime() + 1_000))).toEqual({
            allowed: true,
            state: { windowStartedAt: now, attempts: 3 },
        });
    });
    it('blocks the fourth attempt in an active window', () => {
        expect(consumeBookingAttempt({ windowStartedAt: now, attempts: 3 }, new Date(now.getTime() + 1_000)).allowed).toBe(false);
    });
    it('starts a new window after fifteen minutes', () => {
        expect(consumeBookingAttempt({ windowStartedAt: now, attempts: 3 }, new Date(now.getTime() + 15 * 60 * 1000))).toEqual({
            allowed: true,
            state: {
