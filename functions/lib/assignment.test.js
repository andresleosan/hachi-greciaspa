...
import { getShiftWindow, getWeekday, isEmployeeEligible, parseAssignmentTime, reservationsOverlap, selectFirstEligibleEmployee, } from './assignment.js';
const fullWeek = {
    monday: 'full',
    tuesday: 'full',
    wednesday: 'full',
...
describe('assignment domain helpers', () => {
    ...
    it('blocks an employee only for active overlapping reservations assigned to that employee', () => {
        ...
        expect(selectFirstEligibleEmployee([employee()], candidate, [reservation({ empleadoId: 'employee-1', timeSlot: '10:00' })])).toBeNull();
        expect(selectFirstEligibleEmployee([employee()], candidate, [reservation({ empleadoId: 'employee-2', timeSlot: '10:00' })])?.id).toBe('employee-1');
        expect(selectFirstEligibleEmployee([employee()], candidate, [reservation({ empleadoId: 'employee-1', status: 'cancelled', timeSlot: '10:00' })])?.id).toBe('employee-1');
        expect(selectFirstEligibleEmployee([employee()], candidate, [reservation({ empleadoId: 'employee-1', status: 'completed', timeSlot: '10:00' })])?.id).toBe('employee-1');
    });
    ...
    it('returns null when no employee is eligible', () => {
        expect(selectFirstEligibleEmployee([employee({ active: false }), employee({ services: [] })], reservation(), [])).toBeNull();
    });
});
