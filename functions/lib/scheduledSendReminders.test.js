...
import { createFirestoreReminderStore, ReminderStatePersistenceError, runReminderOrchestration, } from './scheduledSendReminders.js';
...
class MemoryReminderStore {
    ...
    async getConfirmedReservation(id) {
        if (this.currentReservation !== undefined)
            return this.currentReservation;
        return this.reservations.find((candidate) => typeof candidate === 'object' &&
            candidate !== null &&
            'id' in candidate &&
            candidate.id === id) ?? null;
    }
    async acquireReminderLock(input) {
        const existing = this.records.get(input.reservaId);
        if (existing?.status === 'sent')
            return { status: 'sent' };
        if (existing?.processingLockUntil &&
    ...
    async updateReminder(id, patch, processingToken) {
        ...
        if (current?.processingToken !== processingToken)
            return false;
        if (current)
    ...
    replaceLease(id, processingToken) {
        const current = this.records.get(id);
        if (!current)
            throw new Error('Missing reminder record');
        this.records.set(id, {
            ...current,
            status: 'pending',
            processingToken,
...
describe('scheduled reminder orchestration', () => {
    ...
    it('allows only one concurrent invocation to acquire the active lock', async () => {
        ...
        store.reservations = [reservation()];
        let sends = 0;
        const run = () => runReminderOrchestration({
            store,
            secret: 'resend-test-secret',
            now: NOW,
            providerFactory: () => ({
    ...
    it('does not convert a successful provider call into a provider failure when state persistence fails', async () => {
        ...
        let providerCalls = 0;
        await expect(runReminderOrchestration({
            store,
            secret: 'resend-test-secret',
            now: NOW,
            providerFactory: () => ({
    ...
    it('rejects a retry before backoff and allows it after nextAttemptAt', async () => {
        ...
        expect(providerCalls).toBe(1);
        expect(store.records.get('reservation-1')?.nextAttemptAt?.toMillis()).toBe(NOW.getTime() + 2_000);
        await runReminderOrchestration({
            store,
            secret: 'resend-test-secret',
    ...
    it('uses the Firestore transaction seam to create zero-attempt state and acquire one lock', async () => {
        ...
        const store = createFirestoreReminderStore(fake);
        const input = {
            reservaId: 'reservation-1',
            scheduledFor: new Date(NOW.getTime() + 24 * 60 * 60 * 1000),
            now: NOW,
            lockUntil: new Date(NOW.getTime() + 10 * 60 * 1000),
            scheduledForTimestamp: Timestamp.fromDate(new Date(NOW.getTime() + 24 * 60 * 60 * 1000)),
