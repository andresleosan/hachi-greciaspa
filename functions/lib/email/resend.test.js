...
describe('reminder email rendering', () => {
    ...
    it('preserves a dashboard placeholder inside interpolated values', () => {
        ...
        expect(reminderHtml).not.toContain('Servicio https://hachi-greciaspa.vercel.app/dashboard');
        expect(confirmationHtml).toContain('Ana {{dashboardUrl}} &lt;spa&gt;');
        expect(confirmationHtml).not.toContain('Ana https://hachi-greciaspa.vercel.app/dashboard');
    });
    ...
    it('leaves unknown template tokens unchanged', () => {
        expect(renderHtmlTemplate('Hola {{known}} {{unknown}}', { known: 'Ana' })).toBe('Hola Ana {{unknown}}');
    });
});
