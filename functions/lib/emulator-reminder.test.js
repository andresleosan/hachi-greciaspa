...
import { createFirestoreReminderStore, runReminderOrchestration, } from './scheduledSendReminders.js';
...
function isIPv6Loopback(host) {
    ...
    if (omittedGroups < 0 || (host.includes('::') && omittedGroups === 0))
        return false;
    const groups = [
        ...leftGroups,
...
const testCredential = cert(testServiceAccount);
const emulatorEnabled = isLocalEmulatorHost(process.env.FIRESTORE_EMULATOR_HOST);
const testApp = emulatorEnabled
    ? initializeApp({ credential: testCredential, projectId: PROJECT_ID }, 'task-8-emulator')
    : null;
const db = testApp ? getFirestore(testApp) : null;
function getTestDb() {
    if (!db)
        throw new Error('Firestore emulator is not enabled for this test run');
    return db;
}
class RecordingProvider {
    ...
    retryable;
    calls = [];
    constructor(failuresRemaining = 0, retryable = true) {
    ...
    async sendReminderEmail(input) {
        ...
        if (this.failuresRemaining === 'always' || this.failuresRemaining > 0) {
            if (this.failuresRemaining !== 'always')
...
async function seedReservations(firestore, reservations) {
    await Promise.all(reservations.map((item) => firestore.collection('reservas').doc(item.id).set(item)));
}
async function readReminder(firestore, reservaId) {
    return firestore.collection('recordatorios').doc(reminderDocId(reservaId)).get();
}
async function cleanup(firestore, reservaIds) {
    await Promise.all(reservaIds.flatMap((id) => [
