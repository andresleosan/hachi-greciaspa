...
describe('onReservaConfirmationCreated handler', () => {
    it('passes the created reservation snapshot to the confirmation provider', async () => {
        ...
        await onReservaConfirmationCreatedHandler(event, db, 'resend-test-secret', () => ({ sendConfirmationEmail }));
    ...
    it('returns without sending when the event has no data', async () => {
        ...
        await onReservaConfirmationCreatedHandler({ ...event, data: undefined }, db, 'resend-test-secret', () => ({ sendConfirmationEmail }));
    ...
    it('does not send confirmation for a non-client pending reservation', async () => {
        ...
        await onReservaConfirmationCreatedHandler(confirmedEvent, db, 'resend-test-secret', () => ({ sendConfirmationEmail }));
        expect(sendConfirmationEmail).not.toHaveBeenCalled();
        expect(db.documents.get('confirmaciones/confirmation-reservation-1')).toMatchObject({
            status: 'failed',
    ...
    it('rethrows only a sanitized retry signal after a retryable delivery failure', async () => {
        const db = new TransactionFirestoreFake();
        await expect(onReservaConfirmationCreatedHandler(event, db, 'resend-test-secret', () => ({
