...
import { reservationsOverlap } from './assignment.js';
...
export function parseCreateReservaInput(value) {
    ...
    const appointment = fromZonedTime(`${normalizedDate}T${normalizedTimeSlot}`, TIME_ZONE);
    ...
    if (normalizedMascotaId !== undefined)
        input.mascotaId = normalizedMascotaId;
    if (normalizedNotes !== undefined)
        input.notes = normalizedNotes;
...
function readBookingQuotaState(data) {
    if (!data)
        return null;
...
export async function createReservaHandler(request, db, now) {
    ...
    const outcome = await db.runTransaction(async (transaction) => {
        ...
        if (!quota.allowed) {
            return persistFailure(new HttpsError('resource-exhausted', 'Booking attempts limit reached'));
        }
        if (!parsed.ok)
        ...
        if (appointment.getTime() <= now.getTime()) {
            return persistFailure(new HttpsError('failed-precondition', 'The appointment must be in the future'));
        }
        ...
        const serviceSnapshot = await transaction.get(db.collection('servicios').doc(input.serviceId));
        ...
        if (!serviceName.trim() ||
            ...
            durationMin <= 0 ||
            durationMin > 1440) {
            return persistFailure(new HttpsError('failed-precondition', 'Service catalog data is invalid'));
        }
        ...
        if (input.mascotaId !== undefined && input.mascotaId !== null) {
            const mascotaSnapshot = await transaction.get(db.collection('mascotas').doc(input.mascotaId));
            const mascota = mascotaSnapshot.exists ? mascotaSnapshot.data() : undefined;
            if (!mascotaSnapshot.exists || mascota?.userId !== uid) {
                return persistFailure(new HttpsError('permission-denied', 'The pet does not belong to the user'));
            }
        }
        const activeUserReservationsSnapshot = await transaction.get(db
            .collection('reservas')
            .where('userId', '==', uid)
            .where('status', 'in', ACTIVE_STATUSES));
        if (activeUserReservationsSnapshot.docs.length >= MAX_ACTIVE_RESERVATIONS) {
            return persistFailure(new HttpsError('resource-exhausted', 'Active reservation limit reached'));
        }
        const availabilitySnapshot = await transaction.get(db
            .collection('reservas')
        ...
        if (!existingReservations) {
            return persistFailure(new HttpsError('failed-precondition', 'Existing reservation data is invalid'));
        }
        ...
        if (existingReservations.some((reservation) => reservationsOverlap(reservation, newReservation))) {
            return persistFailure(new HttpsError('failed-precondition', 'The requested time slot is unavailable'));
        }
        ...
        if (input.mascotaId !== undefined)
            reservationData.mascotaId = input.mascotaId;
        transaction.set(guardReference, guardData);
...
export const createReserva = onCall({ enforceAppCheck: process.env.FUNCTIONS_EMULATOR !== 'true' }, async (request) => createReservaHandler(request, getFirestore(), new Date()));
