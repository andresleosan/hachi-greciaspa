...
describe('Resend reminder provider', () => {
  beforeEach(() => {
    ...
    resendMocks.sdkLogError.mockImplementation((...args: unknown[]) => {
      console.error(...args)
    })
  ...
  it('marks timeout and server failures as retryable without exposing provider details', async () => {
    resendMocks.send.mockRejectedValueOnce(
      Object.assign(new Error('request timed out with resend_test_secret'), {
        code: 'ETIMEDOUT',
      }),
    )
    const provider = createResendProvider('resend_test_secret')

    const timeoutError = await provider.sendReminderEmail(input).catch(
      (error: unknown) => error,
    ...
    const serverError = await provider.sendReminderEmail(input).catch(
      (error: unknown) => error,
    ) as EmailProviderError
  ...
  it('suppresses SDK error logging at the Resend instance boundary', async () => {
    ...
    try {
      const provider = createResendProvider('resend_test_secret')
      await expect(provider.sendReminderEmail(input)).rejects.toBeInstanceOf(
        EmailProviderError,
      )
