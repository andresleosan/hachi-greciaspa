...
import {
  ...
  parseCreateReservaInput,
  type CreateReservaInput,
} from './createReserva.js'
import {
...
type QueryFilter = { field: string; operator: QueryOperator; value: unknown }
type DocumentReferenceFake = { kind: 'document'; path: string; id: string }
type QueryReferenceFake = {
  kind: 'query'
  collection: string
  filters: QueryFilter[]
  where: (field: string, operator: QueryOperator, value: unknown) => QueryReferenceFake
}
type ReferenceFake = DocumentReferenceFake | QueryReferenceFake
type StagedWrite = {
  kind: 'set' | 'create' | 'update'
  path: string
  data: StoredDocument
}
...
class TransactionFirestoreFake {
  ...
  readonly created: Array<{ path: string; data: StoredDocument }> = []
  readonly sets: Array<{ path: string; data: StoredDocument }> = []
  readonly updates: Array<{ path: string; data: StoredDocument }> = []
  readonly queries: QueryFilter[][] = []
  transactionCalls = 0
  ...
  collection(name: string) {
    return {
      doc: (id?: string): DocumentReferenceFake => {
      ...
      where: (
        ...
        value: unknown,
      ): QueryReferenceFake => this.query(name, [{ field, operator, value }]),
    }
  }

  private query(collection: string, filters: QueryFilter[]): QueryReferenceFake {
    return {
      kind: 'query',
      collection,
      filters,
      where: (field, operator, value) => this.query(
        collection,
  ...
  async runTransaction<T>(callback: (transaction: unknown) => Promise<T>): Promise<T> {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      const transaction = {
        get: async (reference: ReferenceFake) => {
        ...
        set: (reference: DocumentReferenceFake, data: StoredDocument) => {
          stagedWrites.push({ kind: 'set', path: reference.path, data: { ...data } })
        },
        create: (reference: DocumentReferenceFake, data: StoredDocument) => {
          stagedWrites.push({ kind: 'create', path: reference.path, data: { ...data } })
        },
        update: (reference: DocumentReferenceFake, data: StoredDocument) => {
      ...
      try {
        ...
        for (const write of stagedWrites) {
          ...
          this.documents.set(
            write.path,
            write.kind === 'update' ? { ...current, ...write.data } : { ...write.data },
          )
        ...
        if (!(error instanceof TransactionConflictError) || attempt === 4) throw error
      }
    }
...
type DirectTransactionFake = {
  get: (reference: DocumentReferenceFake) => Promise<{
    exists: boolean
    data: () => StoredDocument | undefined
  }>
  set: (reference: DocumentReferenceFake, data: StoredDocument) => void
  create: (reference: DocumentReferenceFake, data: StoredDocument) => void
  update: (reference: DocumentReferenceFake, data: StoredDocument) => void
}
...
describe('TransactionFirestoreFake', () => {
  it('preserves the first version observed when a document is read twice', async () => {
    ...
    await expect(
      firestore.runTransaction(async (transaction) => {
        const directTransaction = transaction as DirectTransactionFake
        attempts += 1
        await directTransaction.get(sharedReference)
        if (attempts === 1) firestore.seed(sharedReference.path, { version: 2 })
        await directTransaction.get(sharedReference)
  ...
  it('discards staged writes from a transaction attempt that loses a conflict', async () => {
    ...
    const firstAttemptReadyPromise = new Promise<void>((resolve) => {
...
function request(
  data: unknown,
  uid = 'user-1',
  token: Record<string, unknown> = { email: 'token@example.com', name: 'Token Name' },
): CallableRequest<CreateReservaInput> {
  return {
    data,
    auth: uid ? { uid, token } : undefined,
  } as unknown as CallableRequest<CreateReservaInput>
}

function rescheduleRequest(
  data: RescheduleReservaInput,
  uid = 'user-2',
): CallableRequest<RescheduleReservaInput> {
  return request(data, uid) as unknown as CallableRequest<RescheduleReservaInput>
}

function readyFirestore(): TransactionFirestoreFake {
  const firestore = new TransactionFirestoreFake()
...
function seedReservation(
  firestore: TransactionFirestoreFake,
  id: string,
...
describe('createReservaHandler canonical snapshot', () => {
  it('creates a canonical snapshot from auth, profile, pet, and active catalog data', async () => {
    ...
    await expect(
      createReservaHandler(
        request({ ...validInput, mascotaId: 'pet-1', notes: ' Leave by noon ' }),
        firestore as unknown as Firestore,
        NOW,
      ),
    ).resolves.toEqual({
...
describe('createReservaHandler ownership and quota', () => {
  it('rejects a pet owned by another user and persists the consumed attempt', async () => {
    ...
    await expect(
      createReservaHandler(
        request({ ...validInput, mascotaId: 'pet-foreign' }),
        firestore as unknown as Firestore,
        NOW,
      ),
  ...
  it('persists an attempt when the input is invalid', async () => {
    ...
    await expect(
      createReservaHandler(
        request({ ...validInput, date: '2026-02-30' }),
        firestore as unknown as Firestore,
        NOW,
      ),
...
describe('createReservaHandler availability and concurrency', () => {
  it('rejects an active overlapping reservation for the same service and date', async () => {
    ...
    await expect(
      createReservaHandler(
        request({ ...validInput, timeSlot: '13:30' }),
        firestore as unknown as Firestore,
        NOW,
      ),
  ...
  it('rejects a same-day slot that is already in the past in Mexico City', async () => {
    ...
    await expect(
      createReservaHandler(
        request({ ...validInput, date: '2026-08-07', timeSlot: '09:59' }),
        firestore as unknown as Firestore,
        NOW,
      ),
    ).rejects.toMatchObject({ code: 'failed-precondition' })
  ...
  it('serializes concurrent attempts for the same user and slot', async () => {
    const firestore = readyFirestore()
    const first = createReservaHandler(
      request(validInput),
      firestore as unknown as Firestore,
      NOW,
    )
  ...
  it('serializes a create and reschedule overlap for different users', async () => {
    ...
    firestore.beforeCommit = async () => {
      ...
      if (arrivals <= 2) await bothTransactionsReady
    }
    ...
    const second = rescheduleReservaHandler(
      rescheduleRequest({
    ...
    expect(
      [...firestore.documents.values()].filter(
        (document) =>
          document.serviceId === 'service-1' &&
          document.date === '2026-08-10' &&
          (document.status === 'pending' || document.status === 'confirmed'),
      ),
    ).toHaveLength(1)
  })
})
