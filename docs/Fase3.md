...
>
> **Pendiente de verificación externa:** rechazo productivo de App Check, dominio y `RESEND_API_KEY` en Secret Manager/Resend, modo de notificación real/pronosticado y destinatarios aprobados, backups, observabilidad, rollback operativo, autorización de producción, despliegue y browser QA contra producción. La habilitación de App Check con reCAPTCHA v3 en Firebase Console fue confirmada por el operador el 2026-08-09; hora no informada; fuente: confirmación del operador. Cuenta de facturación, Blaze y budget fueron confirmados por el operador; los umbrales `10%/50%/100%` son visibles/confirmados, pero el modo y destinatarios siguen pendientes. Durante esta intervención no se ejecutaron acciones productivas.
...
- [ ] Permanecen pendientes el rechazo productivo de App Check, Secret Manager/Resend, modo de notificación real/pronosticado y destinatarios aprobados, deploy con autorización explícita, verificación del lock privado en producción, backups, observabilidad, rollback operativo no destructivo y browser QA productivo. La habilitación de App Check en Console fue confirmada por el operador el 2026-08-09; cuenta de facturación, Blaze y budget están confirmados; los umbrales `10%/50%/100%` son visibles/confirmados; hora no informada; fuente: confirmación del operador.
...
- [x] Umbrales visibles/confirmados en `10% ($1)`, `50% ($5)` y `100% ($10)`.
- [ ] Verificar el modo de gasto real/pronosticado y los destinatarios aprobados; no registrar destinatarios completos.
- [x] Blaze y la cuenta de facturación confirmados por el operador.

**Estado operativo 2026-08-09:** cuenta de facturación, Blaze y budget confirmados por el operador; hora no informada por el operador; fuente: confirmación/captura del operador. El budget es mensual, de `$10`, con alcance solo `hachi-greciaspa`, gasto observado `$0.00/$10.00` y umbrales visibles/confirmados en `10%`, `50%` y `100%`. La captura no demuestra el modo de gasto real/pronosticado ni los destinatarios aprobados, que quedan pendientes. El budget es alert-only y no constituye un hard cap.
...
|---|---|
| A (Resend + Functions) | $0 de email en el baseline de Resend; Blaze/pay-as-you-go está activo y la callable agrega lecturas/escrituras de Firestore e índices; estimación operativa esperada `$0–3/mes`, con costo variable pendiente de verificación |
| B (sin nuevos servicios) | $0 |
| C (sin proveedor externo) | $0 incremental; backups < 1 GB = $0 |
| D | $0 |

**Con Blaze/pay-as-you-go activo** (confirmado el 2026-08-09; hora no informada por el operador; fuente: confirmación/captura del operador): el budget mensual de `$10` para `hachi-greciaspa` y sus umbrales visibles `10%`, `50%` y `100%` están confirmados. El modo de gasto real/pronosticado y los destinatarios aprobados siguen pendientes. El budget es alert-only y no impone un límite duro de facturación.
